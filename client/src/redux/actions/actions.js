...
export function getVideogames() {
    return async function (dispatch) {
        try {
            ...
            console.log(error)
            throw new Error('No se pudoeron obtener los videojuegos')
        }
    }
}
export const getInitialVideogames = () => { // los primeros 100 videojuegos
    return async function (dispatch) {
        try {
            ...
            throw new Error('No se pudoeron obtener los videojuegos iniciales')
        }
    }
}
...
export function createGame(data) {
    return async function (dispatch) {
        try {
            const response = await axios.post(`http://locahost:3001/videogames/`, data)
            return dispatch({
                type: CREATE_GAME,
                payload: response
            })
...
export function changePage(page) { // para el paginado
    return {
        type: CHANGE_PAGE,
        payload: page
    }
...
export function getCurrentPages(currentGames) { //obtiene las páginas según la cantidad de videojuegos
    try {
        if (currentGames) {
            let videogames = currentGames
            let max = Math.ceil(videogames.length / 20) //5

            let slicedGames = [[1, videogames.slice(0, 20)]]
            let i = 2

            while (max > 1) { //7>1, 6>1, etc
                slicedGames.push([i, videogames.slice(20 * (i - 1), 20 * i)])
                i++
                max--
            }
...
export function restartCurrentPage(allGames) { //resetea la página actual
    return async function (dispatch) {
        try {
            if (allGames) {
...
export const filterChangeValue = (property, value) => { // para los filtros
    return {
